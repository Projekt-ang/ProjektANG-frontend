...
export default new Vuex.Store({
  ...
  actions: {
    login({ commit }, user) {
      return new Promise((resolve, reject) => {
        commit("auth_request");
        axios({
          url: "http://18.195.242.27:8088/api/authenticate",
          data: user,
          method: "POST"
        })
          .then(resp => {
    ...
    loadBlankTests({ commit }) {
      axios
        .get("http://18.195.242.27:8080/api/BlankInsertTests")
        .then(response => {
    ...
    loadTags({ commit }) {
      axios.get("http://18.195.242.27:8080/tags").then(response => {
  ...
  getters: {
    ...
    getBlankById: state => id =>
      state.blankTests.find(blank => blank.id === id),
    //{
